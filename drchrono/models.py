...
def validate_gender(value):
    v = value.lower()
    if not v in ('m','male','f','female','h','hermaphrodite'):
        raise ValidationError('%(value)s is not a valid gender', params={'value':value})

def validate_appt_status(value):
    if not value in ("", "Arrived", "Checked In", "In Room", "Cancelled", "Complete",
                     "Confirmed", "In Session", "No Show", "Not Confirmed", "Rescheduled"):
        raise ValidationError('%(value)s is not a valid status', params={'value':value})
...
class Doctor(models.Model):
    ...
    def __str__(self):
        f=str(self.first_name)[2:-3]
        l=str(self.last_name)[2:-3]
        return '{} {}'.format(f, l)
...
class Patient(models.Model):
    ...
    gender                     = models.CharField(max_length=15, null=True, validators=[validate_gender])
...
class Appointment(models.Model):
    ...
    reason                     = models.CharField(max_length=64, default="") # also known as Chief Complaint
    status                     = models.CharField(max_length=30, default="", null=True, validators=[validate_appt_status])
    ...
    prior_status               = models.CharField(max_length=30, default="", null=True, validators=[validate_appt_status])
